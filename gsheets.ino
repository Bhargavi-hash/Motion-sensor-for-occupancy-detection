...
// /trigger/PIR_Temp_Readings/json/with/key/bBrYNjF7M_qEMQHQET1lVMThZTjIRSwKgx-7JnDoovQ
//*** time n date**********
...
const char* resource = "/trigger/PIR_Temp_Readings/json/with/key/bBrYNjF7M_qEMQHQET1lVMThZTjIRSwKgx-7JnDoovQ";
...
void makeIFTTTRequest() {
  ...
  if(!!!IFTTTclient.connected()) {
    Serial.println("Failed to connect IFTTT...");
  }
  ...
  String jsonObject = String("{\"value1\":\"") + pirState + "\",\"value2\":\"" + gridEyeTemperature
                      + "\",\"value3\":\"" + grid_sensor.pixelMatrix + "\"}";
  ...
  if(!!!IFTTTclient.available()) {
    Serial.println("No IFTTT response...");
  }
  while(IFTTTclient.available()){
    Serial.write(IFTTTclient.read());
  }
  
  Serial.println("\nclosing IFTTT connection");
  IFTTTclient.stop(); 
}
...
// if duration after "motion stopped"  and next line is >=10 sec . then it is considered as motion stopped really, otherwise motion is not really stopped.
  ...
  gridEyeTemperature = grid_sensor.thermistorTemperature;
